...
pnbd_dyncov_DECT_egg <- function(clv.fitted, predict.number.of.periods, prediction.end.date, continuous.discount.factor, spending.table){
  ...
  d <- CLVTools:::clv.time.interval.in.number.tu(clv.time=clv.time,
                                      interv = interval(start = clv.time@timepoint.estimation.end,
                                                        end =  CLVTools:::clv.time.ceiling.date(clv.time = clv.time,
                                                                                     timepoint = clv.time@timepoint.estimation.end)))
  
  # S ---------------------------------------------------------------------------------------------------------

  dt.ABCD <- CLVTools:::pnbd_dyncov_ABCD(clv.fitted = clv.fitted, prediction.end.date = prediction.end.date)
  ...
  #Add spending form extended Gamma/Gamma and multiply S with the predicted spending
  dt.ABCD <- merge(dt.ABCD, spending.table, by=c("Id","Cov.Date"))
  dt.ABCD[, S := S * pred.price ]
  
  #dt.S <- dt.ABCD[, list(S = sum(S), pred.price =mean(pred.price)), keyby="Id"]
  ...
  dt.palive <- CLVTools:::pnbd_dyncov_palive(clv.fitted = clv.fitted)
  dt.result[dt.palive, palive := i.palive, on = "Id"]

  dt.result[, F1 := delta^(s-1) * ((r+x)*  (beta_0+DkT)^s)   /  (Bksum + alpha_0)]
  #dt.result[dt.S, `:=`(S = i.S , predicted.mean.spending = i.pred.price), on = "Id"]
  dt.result[dt.S, S := i.S, on = "Id"]
  
  dt.result[, predicted.CLV := palive * F1 * S]
...
.f_confhypergeo_secondkind <- function (a, b, z)
{
  ...
  return(Re(z^(-a)* CLVTools:::vec_gsl_hyp2f0_e(vA = a, vB = 1 + a - b, vZ = -z^(-1))$value))
}
